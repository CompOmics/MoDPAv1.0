...
def process_chunk(data: pd.DataFrame, chunk_path: str) -> bool:
    ...
    grouped_counts = grouped_counts.collect(engine="streaming")
    totals_per_location = totals_per_location.collect(engine="streaming")
...
# In[]: main
def get_relative_ptms(in_path):
    expected_suffix = "_Peptidoforms_counts_mapped.csv.gz"
    if not in_path.endswith(expected_suffix):
        raise ValueError(f"Input file must end with {expected_suffix!r}.")
    out_path = in_path.removesuffix(
        expected_suffix
    ) + "_PTMs_counts_relative.csv.gz"
    ...
    data_chunks = lf.collect(engine="streaming").partition_by("file_name")

    try:
        os.makedirs(tmp_dir_path)
    except FileExistsError:
        sys.exit(f"Temporary folder already exists: {tmp_dir_path}. Please remove it before running the script.")
    ...
    if not chunk_path_list:
        raise ValueError("No MS runs contained usable PTM data after filtering.")
    else:
        print('# files with usable data:', len(chunk_path_list))
    final_data = pl.concat([
        pl.read_csv(path, schema_overrides=schema)
        for path in chunk_path_list
    ]) 
    ...
    try:
        ...
        print('Warning! could not remove temporary folder counts-per-msrun')
    
    final_data = final_data.rename({'LeadProt':'UniAcc'})
    final_data.to_pandas().to_csv(
...
def main():
    ...
    args = parse_cli() 
    get_relative_ptms(args.input_path)
