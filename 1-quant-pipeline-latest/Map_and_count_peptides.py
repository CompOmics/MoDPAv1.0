...
# MAIN CODE 
# ---------------
def get_peptidoforms_counts_mapped(
    peptidoforms_ids_path,
    peptidoforms_counts_path,
    fasta_path,
    p=date.today().isoformat(),
    t=-1
):
    print('The output prefix is:', p)
    if t > 0:
        os.environ["POLARS_MAX_THREADS"] = f"{t}" # to stop polars from using all available memory
    ...
    mapped_peptidoforms_counts = add_psm_counts(peptidoforms, peptidoforms_counts_path)
    ...
    out_path = f'{p}_Peptidoforms_counts_mapped.csv.gz'
...
def main():
    ...
    args = parse_cli()
    get_peptidoforms_counts_mapped(
